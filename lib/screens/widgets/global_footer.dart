...
// import 'package:voidmusic/providers/overlay_provider.dart';

// NEW IMPORTS FOR MUZO FOOTER
import 'dart:ui';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
...
// ─── ORIGINAL FOOTER CONSTANTS (COMMENTED OUT) ────────────────────────────────────────────────────────
...
// ─── Constants for new footer ────────────────────────────────────────────────────────
const double _kDesktopSidebarWidth = 80.0;
const double _kOuterBottomPadding = 6.0;

// ORIGINAL GlobalFooter - COMMENTED OUT
/*
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL MUZO-STYLE MOBILE FOOTER OVERLAY (COMMENTED OUT)
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL SEARCH TEXT FIELD (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL NAV BUTTONS ROW (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      _NavItemData(
        ...
        iconLine: FluentIcons.home_24_regular,
        label: l10n.navHome,
      ),
      _NavItemData(
        branchIndex: 1,
        icon: FluentIcons.library_24_filled,
        iconLine: FluentIcons.library_24_regular,
        label: l10n.navLibrary,
      ),
      _NavItemData(
        branchIndex: 2,
        icon: FluentIcons.search_24_filled,
        iconLine: FluentIcons.search_24_regular,
        label: l10n.navSearch,
      ),
      _NavItemData(
        branchIndex: 3,
        icon: FluentIcons.music_note_2_24_filled,
        iconLine: FluentIcons.music_note_2_24_regular,
        label: l10n.navLocal,
      ),
      _NavItemData(
        branchIndex: 4,
        icon: FluentIcons.folder_zip_24_filled,
        iconLine: FluentIcons.folder_zip_24_regular,
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL DESKTOP MINI PLAYER FOOTER (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL ANIMATED PAGE VIEW (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL VERTICAL NAV BAR (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// ORIGINAL SHARED NAV ITEM BUTTON (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// PREVIOUS DRAFT FOOTER (COMMENTED OUT)
// ─────────────────────────────────────────────────────────────────────────────
...
class _GlobalFooterState extends State<GlobalFooter> {
  ...
  Widget build(BuildContext context) {
    final isMobile = ResponsiveBreakpoints.of(context).isMobile;
    final isPlayerExpanded = context.watch<PlayerOverlayCubit>().state;
                  ...
                  children: [
                    ...
                    if (isMobile)
                      ...
                      Positioned(
                        left: _kDesktopSidebarWidth + 8.0,
                        right: 8.0,
                        bottom: MediaQuery.of(context).viewPadding.bottom + _kOuterBottomPadding,
                        height: 64,
                        child: _DesktopMiniPlayerFooter(
                          navigationShell: widget.navigationShell,
                        ),
...
class _FooterAwareBodyOld extends StatelessWidget {
  const _FooterAwareBodyOld({
    required this.isMobile,
    required this.navigationShell,
  });
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MiniPlayerCubit, MiniPlayerState>(
      builder: (context, miniState) {
        ...
        double footerExtra = _kOuterBottomPadding;
        if (isMobile) footerExtra += 80.0; // Space for Muzo footer
        if (hasMiniPlayer) footerExtra += 56.0;
...
class _MuzoFooterWrapper extends StatelessWidget {
  const _MuzoFooterWrapper({
    required this.navigationShell,
...
class _FooterContent extends StatelessWidget {
  const _FooterContent({required this.navigationShell});
  final StatefulNavigationShell navigationShell;

  @override
  Widget build(BuildContext context) {
    return _AnimatedPageView(navigationShell: navigationShell);
  }
}
*/

// ─────────────────────────────────────────────────────────────────────────────
// EXACT MUZO MOBILE FOOTER NAVIGATION & FLOATING MINI PLAYER
// ─────────────────────────────────────────────────────────────────────────────

class GlobalFooter extends ConsumerStatefulWidget {
  const GlobalFooter({super.key, required this.navigationShell});
  final StatefulNavigationShell navigationShell;

  @override
  ConsumerState<GlobalFooter> createState() => _GlobalFooterState();
}

class _GlobalFooterState extends ConsumerState<GlobalFooter> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  @override
  Widget build(BuildContext context) {
    // ORIGINAL: final isMobile = ResponsiveBreakpoints.of(context).isMobile;
            ...
            child: Scaffold(
              ...
              body: Stack(
                ...
                children: [
                  ...
                  // ── Muzo Mobile Footer Nav & Floating Mini Player ───────
                  /* ORIGINAL INLINE FOOTER RENDERING:
                  if (isMobile) ...[
                    _buildBottomNavBar(context, ref, selectedIndex, isPlayerExpanded),
                    _buildMiniPlayerPositioned(context, isPlayerExpanded),
                  ] else
                  */
                  if (isMobile)
                    MuzoFooter(
    ...
    return Positioned(
      ...
      child: AnimatedOpacity(
        ...
        child: IgnorePointer(
          ...
          child: Row(
            children: [
              // Main pill: Home, Library, Local, Offline OR Search input
              Expanded(
                child: Container(
                  height: 56,
                  decoration: BoxDecoration(
                    ...
                    child: GestureDetector(
                      onTap: () {
                        ...
                        if (isSearchActive) {
                          ...
                          widget.navigationShell.goBranch(0); // Go to Home
                        } else {
                          widget.navigationShell.goBranch(2); // Search index
  ...
  Widget _buildNavButtons(BuildContext context, WidgetRef ref, int selectedIndex) {
    return Row(
      ...
      children: [
        ...
        _buildNavItem(context, ref, FluentIcons.library_24_regular, FluentIcons.library_24_filled, "Library", 1, selectedIndex),
        _buildNavItem(context, ref, FluentIcons.music_note_1_24_regular, FluentIcons.music_note_1_24_filled, "Local", 3, selectedIndex),
        _buildNavItem(context, ref, FluentIcons.arrow_download_24_regular, FluentIcons.arrow_download_24_filled, "Offline", 4, selectedIndex),
      ],
    );
  }
  ...
  Widget _buildMiniPlayerPositioned(
    BuildContext context,
    bool isPlayerExpandedVal,
  ) {
    final isDarkTheme = Theme.of(context).brightness == Brightness.dark;
    final bottomOffset = 76.0 + MediaQuery.of(context).padding.bottom;
    final keyboardVisible = MediaQuery.of(context).viewInsets.bottom > 0;
                  ...
                  child: BackdropFilter(
                    ...
                    child: Container(
                      ...
                      child: MuzoMiniPlayer(
                        currentPageIndex: widget.navigationShell.currentIndex,
                      ),
                    ),
                  ),
                ),
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MiniPlayerCubit, MiniPlayerState>(
      builder: (context, miniState) {
        ...
        double footerExtra = _kOuterBottomPadding;
        if (isMobile) {
          footerExtra = hasMiniPlayer ? 134.0 : 76.0;
        } else if (hasMiniPlayer) {
          footerExtra += 72.0;
        }
