...
class _GlobalFooterState extends State<GlobalFooter>
  ...
  // Reference-exact thresholds matching native_glass_navbar behavior
  // Collapse: 15 px of intentional downward scroll triggers shrink.
  // Expand  : 20 px of upward scroll anywhere on page triggers grow-back.
  // Expand is fractionally longer than shrink as requested.
  static const double _kCollapseDeltaThreshold = 15.0;
  static const double _kExpandDeltaThreshold = 20.0;
  ...
  bool _onScrollNotification(ScrollNotification notification) {
    ...
    // Reference-exact: 100ms debounce to prevent jitter on rapid direction reversals
    _debounceTimer = Timer(const Duration(milliseconds: 100), () {
      // Timer completed - state is stable
    });
            ...
            child: Scaffold(
              ...
              body: Stack(
                ...
                children: [
                  ...
                  _FloatingSleepTimer(navigationShell: widget.navigationShell),
                ],
              ),
            ),
          ),
...
class _FloatingSleepTimer extends StatefulWidget {
  const _FloatingSleepTimer({required this.navigationShell});
  final StatefulNavigationShell navigationShell;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TimerBloc, TimerState>(
      builder: (context, state) {
        ...
        return Positioned(
          left: _position.dx.clamp(8.0, screenWidth - 128),
          top: _position.dy.clamp(8.0, screenHeight - 64),
          child: GestureDetector(
            ...
            onPanEnd: (details) {
              // Clamp position to keep chip visible
              setState(() {
                _position = Offset(
                  _position.dx.clamp(8.0, screenWidth - 128),
                  _position.dy.clamp(8.0, screenHeight - 64),
                );
              });
            },
            child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<MiniPlayerCubit, MiniPlayerState>(
      builder: (context, miniState) {
        ...
        // ── Reference-exact: LiquidGlassLayer Implementation ──
        // Custom implementation with enhanced liquid glass settings matching native_glass_navbar
            ...
            children: [
              ...
              Positioned(
                ...
                child: lgr.LiquidGlass.withOwnLayer(
                  settings: lgr.LiquidGlassSettings(
                    // Reference-exact liquid glass settings
                            ...
                            children: [
                              ...
                              Expanded(
                                child: AnimatedContainer(
                                  ...
                                  child: Transform.translate(
                                    offset: Offset(0, isCollapsed ? -4 : 0), // Reference-exact rise when collapsed
        ...
        // Use custom fallback implementation directly for consistent liquid glass effect
        return customFallbackNav;
      },
    );
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// NAV ITEM BUTTON (horizontal nav bar)
// ─────────────────────────────────────────────────────────────────────────────
      ...
      builder: (overlayContext) => Positioned(
        ...
        child: AnimatedBuilder(
          ...
          child: GlassChip(
            ...
            // Reference-exact jello physics: high stretch + elastic scale
                    ...
                    children: [
                      ...
                      if (isTabActive)
                        AnimatedPositioned(
                          ...
                          height: _kNavBarH - 8,
                          child: Transform.translate(
                            offset: const Offset(0, -2), // Slight rise like reference
                            child: lgr.LiquidGlass.withOwnLayer(
                              settings: const lgr.LiquidGlassSettings(
                                blur: 3,
                                thickness: 28,
                              ...
                              shape: const lgr.LiquidRoundedSuperellipse(borderRadius: 24),
                              glassContainsChild: false,
                              child: Container(
                                decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(24),
                                ...
                                child: ClipRRect(
                                  ...
                                  child: BackdropFilter(
                                    ...
                                    child: Container(
                                      decoration: BoxDecoration(
                                        color: activeAccentColor.withValues(alpha: 0.12),
                                        borderRadius: BorderRadius.circular(24),
                                      ),
                                    ),
                                  ),
